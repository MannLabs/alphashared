...
def process_json_data(json_string, pr, last_commit):
    ...
    for json_item in json_items:
        ...
        if not all(field in json_item for field in required_fields):
            print(
                f"Error: Invalid json_item structure for change_id: {json_item.get('change_id', 'unknown')}"
            )
            continue
        ...
        try:
            ...
            pr.create_review_comment(
                body=comment, commit=last_commit, path=file_name, line=line
            )
...
def main():
    """Main function to process the JSON data and post comments to the PR."""
